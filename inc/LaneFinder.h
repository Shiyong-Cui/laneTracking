...
class LaneFinder {
public:
  LaneFinder(DashboardCamera *cam, vector<int> &window_shape,
             int search_margin = 90, int max_frozen_dur = 15);
  virtual ~LaneFinder();

  Mat score_pixels(const Mat &img);
  int getCode(const string &str);
  float calc_curvature(const vector<Window *> &windows);
  map<string, float> fit_lanes(const vector<Point> &points_left,
                               const vector<Point> &points_right,
                               bool fit_globally = false);
  Mat find_lines(const Mat &img_dashboard);

  // static functions
  static void joint_sliding_window_update(vector<Window *> &windows_left,
                                          vector<Window *> &windows_right,
                                          const Mat &score_img, int margin);
  static float start_sliding_search(vector<Window *> &windows,
                                    const Mat &score_img, int mode = 0);
  static bool strictly_decreasing(vector<float> &list);
  static void filter_window_list(const vector<Window *> &windows,
                                 vector<Window *> &windows_filtered,
                                 vector<int> &indexes,
                                 bool remove_frozen = false,
                                 bool remove_dropped = true,
                                 bool remove_undetected = false);
  ...
  template <typename Real>
  std::vector<Real> interp1(std::vector<Real> &x, std::vector<Real> &y,
                            std::vector<Real> &x_new);
  void Test_Interp1();

  // functions for visualization
  Mat viz_windows(const Mat &score_img, int mode);
  Mat viz_lane(const Mat &undist_img, const DashboardCamera &camera,
               const vector<float> &left_fit_x,
               const vector<float> &right_fit_x, const vector<int> &fit_y);

  static Mat window_image(const vector<Window *> &windows, int opt = 0,
                          const Scalar color = Scalar(0, 255, 0),
                          const Scalar color_frozen = 0.2 * Scalar(0, 255, 0),
                          const Scalar color_droped = Scalar(0, 0, 0));
