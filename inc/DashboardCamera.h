...
public:
  ...
  DashboardCamera(vector<string> chessboard_img_fnames,
                  vector<int> chessboard_size, vector<cv::Point2f> lane_shape,
                  vector<float> scale_correction /*={30 / 720.0, 3.7 / 700}*/);
  // load a camera from a file
  DashboardCamera(const string &file, int image_height, int img_width,
  ...
  // Transforms this camera's images from the dashboard perspective to an
  // overhead perspective.
  Mat warp_to_overhead(const Mat &image);
  // Transforms this camera's images from an overhead perspective back to the
  ...
  float getXMPerPix() const { return m_x_m_per_pix; }

  void setXMPerPix(float xMPerPix) { m_x_m_per_pix = xMPerPix; }

  float getYMPerPix() const { return m_y_m_per_pix; }

  void setYMPerPix(float yMPerPix) { m_y_m_per_pix = yMPerPix; }

protected:
  // Calibrates the camera using chessboard calibration images.
  void calibrate(vector<string> chessboard_img_fnames,
                 vector<int> chessboard_size);
  void computeTransform(const vector<cv::Point2f> &lane_shape,
                        const vector<float> &scale_correction);

  // compute perspective maps
  void perspective_to_maps(const cv::Mat &perspective_mat,
                           const cv::Size &img_size, cv::Mat &mapx,
                           cv::Mat &mapy);
