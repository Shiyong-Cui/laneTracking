...
Window::Window(int level, vector<int> window_shape, vector<int> img_shape,
               float x_init, int max_frozen_dur) {

  if (img_shape.size() < 2)
    cerr << "no image shape" << endl;
  if (window_shape.size() < 2)
    cerr << "no window shape" << endl;
  if (window_shape[1] % 2 == 0)
    cerr << "width must be odd" << endl;
  // Image info
  img_h = img_shape[0];
  img_w = img_shape[1];
...
void Window::update(const Mat &score_img, vector<float> &x_search_range,
                    float min_log_likelihood /*=-40*/) {
  /*  Given a score image and the x search bounds, updates the window position
  to the likely position of the lane.

  If the measurement is deemed suspect for some reason, the update will be
  rejected and the window will be 'frozen', causing it to stay in place. If the
  window is frozen beyond its  `max_frozen_dur` then it will be dropped entirely
  until a non-suspect measurement is made.
  ...
  lane most likely is. :param x_search_range: The (x_begin, x_end) range the
  window should search between in the score image. :param min_log_likelihood:
  The minimum log likelihood allowed for a measurement before it is rejected.
   */

  int w = score_img.cols, h = score_img.rows;
  if (w != img_w || h != img_h)
    cerr << " Window not parametrized for this score_img size" << endl;

  // Apply a column-wise Gaussian filter to score the x-positions in this
  // window's search region
  x_search_range[0] = max(0, int(x_search_range[0]));
  x_search_range[1] = min(int(x_search_range[1]), img_w);

  int x_offset = x_search_range[0];
  Mat search_region = score_img(cv::Range(y_begin, y_end),
                                cv::Range(x_offset, x_search_range[1]));
  ...
  if (maxVal > 0.0) {
    ...
    if (window_magnitude > 0)
      signal_noise_ratio =
          window_magnitude / (window_magnitude + noise_magnitude);
    else
      signal_noise_ratio = 0.0;

    if (signal_noise_ratio < 0.6 ||
        filter->loglikelihood(x_measured) < min_log_likelihood) {
