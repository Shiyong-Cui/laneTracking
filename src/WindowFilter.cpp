...
WindowFilter::WindowFilter(float pos_init /* = 0.0*/, float meas_variance/* = 50*/, float process_variance /* = 1.0*/, \
                            float uncertainty_init /* = pow(2.0, 30)*/): state_dim(2), meansure_dim(1)
{
	...
	m_kalman->errorCovPost = Mat::eye(state_dim, state_dim, CV_32FC1) * uncertainty_init;

	// Measurement noise
	m_kalman->measurementNoiseCov = Mat::eye(meansure_dim, meansure_dim, CV_32FC1) * meas_variance;
	...
	cout << "m_kalman->transitionMatrix" << endl;
	cout << m_kalman->transitionMatrix << endl;

	cout << "m_kalman->measurementMatrix" << endl;
	cout << m_kalman->measurementMatrix << endl;

	cout << "m_kalman->errorCovPost" << endl;
	cout << m_kalman->errorCovPost << endl;

	cout << "m_kalman->measurementNoiseCov" << endl;
	cout << m_kalman->measurementNoiseCov << endl;

	cout << "m_kalman->processNoiseCov" << endl;
	cout << m_kalman->processNoiseCov << endl;
	*/
...
{
	for(int i = 0; i < mag; i++){
		m_kalman->errorCovPost = m_kalman->transitionMatrix * m_kalman->errorCovPost*  m_kalman->transitionMatrix.t() + m_kalman->processNoiseCov;
	}
}

float WindowFilter::loglikelihood(float pos)
{
	Mat temp = m_kalman->measurementMatrix * m_kalman->errorCovPost * m_kalman->measurementMatrix.t() + m_kalman->measurementNoiseCov;
	float var = temp.at<float>(0);
	Mat_<float> state(2,1); state.setTo(Scalar(0));
	state(0) = x_est; state(1) = 0.0;
